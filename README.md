...
* 实时合成语音流：支持Http[s]风格和WebsocketAPI风格，适应不同的工作场景（本月即将发布）
* 背景声效合成：支持背景音替换
* 极速输出语言合成模型：支持10秒~30秒客户语音录制即可输出和客户声音相似的语音模型（本月即将发布）

## 下面是依图语音合成v2版本接口，V1版本接口移步到  v1版本接口请到doc目录下查看README-v1.md
